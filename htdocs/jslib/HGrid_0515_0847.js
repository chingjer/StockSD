...
function ajaxPost(sMode, sPage, sPage_rows, sUrl, sInfo) {
    ...
    xajax.done(
            function (data, status) {
                if (sMode == "GRID" || sMode == "FILTER") {
                    //$("#hgrid").html(data);
                    document.getElementById("hgrid").innerHTML = data;
...
function doFilter() {
    ...
    xx = "";
    objs = $("#filter input[type!=button][type !='radio'][type !='checkbox']");
    for (i = 0; i < objs.length; i++)
        xx += "," + objs.eq(i).val();
    objs = $("#filter :checked");
    ...
    objs = $("#hgrid #PAGE_ROWS");
    if (objs.length > 0)
        page_rows = $("#hgrid #PAGE_ROWS").val();
    //else
    //    alert("page_rows not found");
    sUrl = document.getElementById("URL").value;
    //alert("sUrl="+sUrl);
    ajaxPost("GRID", "1", page_rows, sUrl, "");
}
/**
...
function doNextPage() {
    ...
    oPage_rows = document.getElementById("PAGE_ROWS");
    oNum_rows = document.getElementById("NUM_ROWS");
    sUrl = document.getElementById("URL").value;
    maxPage = Math.ceil(oNum_rows.value / oPage_rows.value);
    if (oPage.value < maxPage) {
        oPage.value = parseInt(oPage.value) + 1;
        ajaxPost("GRID", oPage.value, oPage_rows.value, sUrl, "");
